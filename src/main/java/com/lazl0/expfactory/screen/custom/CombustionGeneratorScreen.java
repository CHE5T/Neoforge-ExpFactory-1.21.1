...
public class CombustionGeneratorScreen extends AbstractContainerScreen<CombustionGeneratorMenu> {
    private static final ResourceLocation GUI_TEXTURE = ResourceLocation.fromNamespaceAndPath(ExpFactory.MODID, "textures/gui/combustion_generator/combustion_generator_gui.png");
    ...
    private static final ResourceLocation FIRE_TEXTURE = ResourceLocation.fromNamespaceAndPath(ExpFactory.MODID, "textures/gui/burn_progress.png");
    ...
    protected void renderBurnProgress(GuiGraphics guiGraphics, int x, int y) {
        int burn_progress = menu.getScaledBurnProgress();
        int blit_height = 14;//It's a square
        if(menu.isBurning()) {
            guiGraphics.blit(FIRE_TEXTURE, x + 80, y + 22 + (blit_height - burn_progress), 0, blit_height - burn_progress, blit_height, burn_progress, blit_height, blit_height);
        }
    }
    ...
    protected void renderEnergyBar(GuiGraphics guiGraphics, int x, int y){
        ...
        int barHeight = 67;
        if(scaledEnergy > 0){
            guiGraphics.blit(ENERGY_BAR_TEXTURE, x + 152, y + 8 + (barHeight - scaledEnergy), 0, barHeight - scaledEnergy, 16, scaledEnergy, 16, barHeight);
        }
    }
    ...
    protected void renderTooltip(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        int maxEnergy = this.menu.data.get(1);

        if(isMouseOver(mouseX, mouseY, x + 152, y + 8, 16, 67)){
